...
def api_instructor_dashboard():
    ...
    Supports ?lite=1 to skip heavy charts for faster load.
    Includes simple in-memory caching (per instructor).
    """
    user = session.get("user")
    ...
    try:
        ...
        # Only fetch heavy charts if not lite
        if not lite:
            data["charts"] = {
...
def view_code():
    """
    Serve code content either from local file or remote URL (e.g. Cloudinary raw URL)
    """
    import requests
    ...
    try:
        # ✅ If it's a Cloudinary or external URL
        ...
        # ✅ Else read from local submitted_codes folder
        import os
...
def submit_code():
    """
    Handles code submission request and returns JSON response.
    Works with both frontend fetch() and browser form submissions.
    """
    ...
    try:
        ...
        return (
            jsonify(
                {"status": "error", "message": "Evaluation failed", "details": result}
            ),
            500,
        )
...
def profile(user_id):
    ...
    target_role = request.args.get(
        "role", session_role
    )  # ✅ allow ?role=student/instructor
    ...
    if target_role == "student":
        ...
        profile_data = pdata.get_basic_profile()
        user_info = profile_data.get("user")
        profile = profile_data.get("profile")
        pdata.close()
        return render_template("profile.html", user=user_info, profile=profile)

    elif target_role == "instructor":
        pdata = InstructorProfileData(user_id)
        profile_data = (
            pdata.get_basic_profile()
        )  # ✅ now only basic info for instructor
        return render_template(
            "profile.html", user=profile_data["user"], profile=profile_data["profile"]
        )
...
def performance_history(user_id):
    ...
    if viewer_role == "admin":
        # detect if viewing instructor or student
        ...
        if target_role == "instructor":
            ...
            completed_assignments = perf_data.get("managed_assignments", {})
        else:
            pdata = StudentProfileData(user_id)
            perf_data = pdata.get_performance_data()
            user_info = pdata.get_user_info()
            completed_assignments = perf_data.get("completed_assignments", {})

        pdata.close()
        report = perf_data.get("report", {})

    # 👨‍🏫 Instructor: can view own + student performance
...
def admin_dashboard():
    try:
        ...
        # ✅ NEW: Fetch all users for management table
        conn = get_connection()
        with conn.cursor(dictionary=True) as cursor:
            cursor.execute(
                "SELECT user_id, first_name, last_name, email, role FROM user_profile ORDER BY user_id ASC"
            )
...
def instructor_students():
    """Allow instructors to view students (similar to admin view, but restricted role)."""
    user_session = session.get("user")
    ...
    try:
        # Same analytics function used by admin
        data = UserAnalytics.list(role="student", search=search, sort=sort)

        # (Optional enhancement) filter students belonging to this instructor:
        # instructor_id = user_session["user_id"]
        # data = [s for s in data if s.get("instructor_id") == instructor_id]
