...
# keep using your retry decorator defined above in this file
def is_html(s: str) -> bool:
    # tiny helper: treat plain text as <pre> for nicer formatting
    return "<" in s and ">" in s


class EmailDelivery:
    """
    Unified Email Delivery for OTPs, notifications, and broadcasts.
    - Uses Brevo API (preferred)
    - Falls back to SMTP if API key missing or API call fails
    """
    ...
    def __init__(self):
        ...
        # Prefer Brevo API
        self.api_key = os.getenv("BREVO_API_KEY") or getattr(
        ...
        self.smtp_server = os.getenv(
            "EMAIL_SMTP_SERVER",
            getattr(Config, "EMAIL_SMTP_SERVER", "smtp-relay.brevo.com"),
        )
        self.smtp_port = int(
            os.getenv("EMAIL_SMTP_PORT", getattr(Config, "EMAIL_SMTP_PORT", 587))
        )
        self.smtp_user = os.getenv(
            "EMAIL_SMTP_LOGIN", getattr(Config, "EMAIL_SMTP_LOGIN", None)
        )
        ...
        self.logger = logging.getLogger("EmailDelivery")

        # Warn if missing keys
        if not self.api_key:
            self.logger.warning(
                "⚠️ BREVO_API_KEY not found — using SMTP fallback for notifications."
            )

    def _send_via_api(
        self, to_email: str, subject: str, body: str, timeout: int = 10
    ) -> bool:
        """Try sending email via Brevo REST API."""
        try:
            if not self.api_key:
                return False

            payload = {
                "sender": {"name": "AIYuga Bharat", "email": self.sender_email},
                "to": [{"email": to_email}],
                "subject": subject or "Notification",
                "htmlContent": (
                    body if is_html(body or "") else f"<pre>{body or ''}</pre>"
                ),
            }
            headers = {
            ...
            if resp.status_code in (200, 201, 202):
                self.logger.info(f"✅ Email via Brevo API sent to {to_email}")
                return True

            self.logger.warning(f"⚠️ Brevo API error: {resp.status_code} → {resp.text}")
            return False
    ...
    def _send_via_smtp(self, to_email: str, subject: str, body: str) -> bool:
        """Fallback SMTP send using Brevo relay (or other configured SMTP)."""
        try:
            import smtplib
            from email.mime.text import MIMEText
            from email.mime.multipart import MIMEMultipart
            ...
            msg.attach(MIMEText(body, "html" if is_html(body) else "plain", "utf-8"))
            ...
            self.logger.info(f"✅ Email via SMTP sent to {to_email}")
            return True
    ...
    def send(self, to_email: str, subject: str, body: str, timeout: int = 10):
        """
        Smart send: Try Brevo API first, then fallback to SMTP.
        """
        if not to_email:
            self.logger.warning("⚠️ EmailDelivery: No recipient email provided.")
            return

        if self._send_via_api(to_email, subject, body, timeout):
            return
