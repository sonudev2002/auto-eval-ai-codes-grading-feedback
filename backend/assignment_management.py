...
# example creation
def create_example(assignment_id, example_input, example_output, explanation):
    try:
        conn = get_connection()
        cursor = conn.cursor()
        cursor.execute(
            """
        INSERT INTO example(assignment_id, example_input, example_output, description)
        VALUES (%s, %s, %s, %s)
        """,
            (assignment_id, example_input, example_output, explanation),
        )
        conn.commit()
        return True
    except Exception as e:
...
def create_assignment(
    ...
    try:
        ...
        # ✅ Commit assignment creation
        conn.commit()

        # -----------------------------------------
        # ✅ Auto-update instructor_difficulty_stats
        # -----------------------------------------
        try:
            cursor.execute(
                """
                INSERT INTO instructor_difficulty_stats
                (user_id, difficulty_level, assignment_count, average_score, average_pass_rate, average_feedback_score)
                VALUES (%s, %s, 1, 0, 0, 0)
                ON DUPLICATE KEY UPDATE
                    assignment_count = assignment_count + 1
                """,
                (instructor_id, difficulty_level),
            )
            conn.commit()
            print(
                f"📊 instructor_difficulty_stats updated for instructor {instructor_id}, difficulty {difficulty_level}"
            )
        except Exception as e:
            print("⚠️ instructor_difficulty_stats update skipped:", e)
...
def upload_assignment(form_data, csv_file=None):
    try:
        ...
        if not assignment_id:
            print("Upload Assignment Error: Could not create assignment")
            return False

        # --- ✅ Step 4: Add examples (Input / Output / Explanation) ---
        inputs = form_data.get("example_input[]", [])
        outputs = form_data.get("example_output[]", [])
        explanations = form_data.get("example_explanation[]", [])

        for idx in range(max(len(inputs), len(outputs), len(explanations))):
            inp = (inputs[idx] if idx < len(inputs) else "").strip()
            out = (outputs[idx] if idx < len(outputs) else "").strip()
            exp = (explanations[idx] if idx < len(explanations) else "").strip()

            if inp or out or exp:
                create_example(assignment_id, inp, out, exp)
...
def update_assignment_backend(form_data):
    ...
    form_data keys:
      ...
      - examples (list[dict], optional)
          each dict = {input, output, explanation}
      - test_cases (list[dict], optional)
          each dict = {input, expected}
    ...
    try:
        ...
        # --- 2. Hint ---
        hint = (form_data.get("hint") or "").strip()
        if hint:
            cursor.execute(
        ...
        # --- 3. Due Date ---
        due = (form_data.get("due_date") or "").strip()
        if due:
            cursor.execute(
            ...
            for ex in examples:
                ...
                if inp or out or exp:
                    cursor.execute(
                        ...
                        VALUES (%s, %s, %s, %s)
                        """,
                        (assignment_id, inp, out, exp),
                    )
            updated_fields.append("examples")

        # --- 5. Replace Test Cases ---
        ...
        if not updated_fields:
            print("No updates provided for assignment:", assignment_id)
            return False

        conn.commit()
        print(f"✅ Updated fields for assignment {assignment_id}: {updated_fields}")
        return True
    ...
    def assignment_detail_by_id(ass_id, user_id=None, role=None):
        ...
        try:
            with Code_editor._get_cursor() as cursor:
                ...
                # 2️⃣ Corrected Example Fetch — use proper DB columns
                cursor.execute(
                    """
                    SELECT example_id, example_input, example_output, description AS explanation
                    FROM example
...
class AssignmentsStudent:
    ...
    def get_dashboard_assignment_detail():
        ...
        query = """
            SELECT 
                ...
                d.difficulty_types AS difficulty_level,
                a.due_date,
                COALESCE(sas.status, 
                    CASE
                        WHEN NOW() < a.due_date THEN 'Pending Submission'
            ...
            LEFT JOIN student_assignment_status sas 
                ON a.assignment_id = sas.assignment_id AND sas.user_id = %s
        ...
        try:
            ...
            logger.exception("❌ Failed to fetch dashboard assignments with status")
            return []
    ...
    def get_repository_details():
        """
        Fetch all repositories with question counts.
        """
        query = """
            SELECT 
                r.repository_id,
        ...
        try:
            ...
            logger.exception("❌ Failed to fetch repository details")
            return []
    ...
    def get_assignments_by_repo_detailed(repo_id):
        ...
        try:
            with AssignmentsStudent._get_cursor() as cursor:
                cursor.execute(query, (user_id, repo_id))
                rows = cursor.fetchall()
            ...
            logger.exception(
                "❌ Failed to fetch assignments with status for repo_id=%s", repo_id
            )
            return []
