...
import requests
from backend.db import get_connection  # <-- your db.py connector
...
def load_submissions_from_db() -> Dict[str, str]:
    ...
    Handles both local paths and remote Cloudinary URLs.
    """
    import requests
    ...
    for row in cursor.fetchall():
        ...
        path = str(row.get("code_path") or "").strip()
        ...
        # 🟢 Case 1: Remote URL (Cloudinary / HTTPS)
        if path.startswith("http://") or path.startswith("https://"):
            try:
                ...
                logger.warning(
                    "Skipping submission %s: failed to fetch URL %s (%s)", sid, path, e
                )
            continue

        # 🟢 Case 2: Local path
        # Normalize relative paths → absolute
...
def load_detector(
    ...
    # Trust your own checkpoint: allow full pickle load
