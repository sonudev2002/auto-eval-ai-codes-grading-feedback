...
# ----------------------------
class DBConnection:
    """Context-managed database connection that automatically consumes unread results safely."""
    ...
    def __exit__(self, exc_type, exc_val, exc_tb):
        try:
            # ✅ Consume unread results if any before commit
            try:
                if hasattr(self.cursor, "_connection") and getattr(
                    self.cursor._connection, "unread_result", False
                ):
                    self.cursor.fetchall()
            except Exception:
                pass  # safely ignore if nothing to read
            ...
            logging.error(f"❌ Commit/Rollback failed in DBConnection: {e}")
        finally:
            try:
                self.cursor.close()
            except Exception:
...
class SystemAnalytics(BaseAnalytics):
    ...
    def save_snapshot(self, data: Dict[str, Any]) -> bool:
        """
        Save a system analytics snapshot to the database.
        Inserts one row into `system_statistics`.
        """
        try:
            with DBConnection() as cur:
                cur.execute(
                    ...
                    (total_students, total_instructors, total_assignments,
                    active_users_today, average_score, total_submissions, new_users_last_week, grade_distribution)
            ...
            logger.info("✅ System snapshot saved successfully")
            return True

        except Exception as e:
            logger.exception("❌ save_snapshot failed: %s", str(e))
            return False
